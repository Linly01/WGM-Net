...
class MAE_RMSE_Loss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        rmse = torch.sqrt(torch.mean((pred - true) ** 2) + 1e-6)
        return self.alpha * mae + (1 - self.alpha) * rmse

class MAE_Relative_Loss(nn.Module):
    def __init__(self, lambda_rel=0.1, eps=1.0):
        super().__init__()
    ...
    def forward(self, pred, true):
        ...
        mae = torch.mean(torch.abs(pred - true))
        rel_err = torch.abs(pred - true) / (true + self.eps)
        return mae + self.lambda_rel * rel_err.mean()
...
args.add_argument('--dataset', default='PEMSD4', type=str)  # NYCBike2 NYCBike1 NYCTaxi
...
args.add_argument('--adj_file', type=str, default='/root/data/PeMS04/adj_pems04.pkl')

args1 = args.parse_args()

#get configuration
config_file = '/root/config_file/{}/{}_{}.conf'.format(args1.type,args1.dataset, args1.model)
...
if args.loss_func == 'mask_mae':
    ...
    # loss = HuberLoss(delta=1.0).to(args.device)
    # loss = MAE_Relative_Loss(lambda_rel=0.3, eps=5).to(args.device)
...
if args.mode == 'train':
    ...
    model.load_state_dict(torch.load('/root/pre-trained/{}/{}.pth'.format(args.type,args.dataset)))
