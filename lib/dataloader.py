    ...
    if normalizer == 'max01':
        ...
        if column_wise:
           ...
           # std = data.std(axis=0, keepdims=True)
            mean = data.mean(axis=(0, 1), keepdims=True)  # shape: (1, 1, 307, 1)
            std = data.std(axis=(0, 1), keepdims=True)  # shape: (1, 1, 307, 1)
        else:
            mean = data.mean()
            std = data.std()
...
def get_dataloader(args, normalizer = 'std', tod=False, dow=False, weather=False, single=True):
    """
        加载交通数据并添加时间特征（一天中的时间 + 星期几 + 节假日）
        节假日编码规则：
          0 = 工作日
          1 = 周末
          2 = 法定节假日（按美国 2018 节假日）
        """
    #load raw st dataset
    data = load_st_dataset(args.dataset)        # B, N, D
    print("原始数据形状 (时间步, 节点数, 原始特征维度):", data.shape)
    # #normalize st data
    # data, scaler = normalize_dataset(data, normalizer, args.column_wise)

    L, N, F = data.shape
    print("原始数据特征维度数量 F =", F)  # 直接打印原始特征维度
    ...
    # 创建日期序列 (2018-01-01 开始，每5分钟一个时间步)
    dates = pd.date_range(start='2018-01-01', periods=L, freq='5T')
    ...
    # 初始化全为 0 (工作日)
    holiday_feature = np.zeros(L, dtype=int)
    # 周末 → 1
    holiday_feature[(dates.weekday >= 5)] = 1
    # 法定节假日 → 2
    holiday_feature[np.isin(dates.normalize(), us_holidays_2018)] = 2
    # 扩展维度 (L, N, 1)
    holiday_feature = np.tile(holiday_feature[:, None], (1, N)).reshape(L, N, 1)
    # weather_feature = data[..., 5]
    ...
    x_holiday, y_holiday = Add_Window_Horizon(holiday_feature, args.lag, args.horizon, single)  # 新增
    #x, y = np.concatenate([x,x_day,x_week], axis=-1), np.concatenate([y,y_day,y_week], axis=-1)
    # x_weather, y_weather = Add_Window_Horizon(weather_feature, args.lag, args.horizon, single)
    # x_weather = x_weather[..., None]
    # y_weather = y_weather[..., None]
    # 拼接所有时间相关特征
    x = np.concatenate([x, x_day, x_week, x_holiday], axis=-1)
    y = np.concatenate([y, y_day, y_week, y_holiday], axis=-1)
